...
void	CRenderer::endDisplays() {
	int	i;
    error(CODE_SYSTEM, "ENDING DISPLAYS %d \n", 42);
...
// KOS RABAK
void* my_display_start(const char *name,int width,int height,int numSamples,const char *samples,TDisplayParameterFunction findParameter){
    error(CODE_SYSTEM, "OUR DISPLAY START FUNCTION %d \n", 42);

    return displayStart(name, width, height, numSamples, samples, findParameter);
}
int my_display_data(void* p1, int height, int width, int p2, int p3, float* probably_data){
//    error(CODE_SYSTEM, "OUR DISPLAY DATA FUNCTION %d \n", 42);

    return displayData(p1, height, width, p2, p3, probably_data);
}

void my_display_finish(void* p4){
    error(CODE_SYSTEM, "OUR DISPLAY FINISH FUNCTION %d \n", 42);
    
    return displayFinish(p4);
}
...
void	CRenderer::computeDisplayData() {
    ...
    error(CODE_SYSTEM, "Number of display devices %d \n", i);
...
//                datas[numDisplays].finish		=	(TDisplayFinishFunction) cDisplay->finishFunction;
                datas[numDisplays].module		=	(void *) outDevice;
                datas[numDisplays].start		=	(TDisplayStartFunction) &my_display_start;
                datas[numDisplays].data			=	(TDisplayDataFunction)  &my_display_data;
                datas[numDisplays].rawData		=	NULL;
                datas[numDisplays].finish		=	(TDisplayFinishFunction) &my_display_finish;
			} else {
